...
**Вариант B (Decorator pattern)** — лучший выбор:

1. **Консистентность** — проект уже использует decorator для трейсинга (`TracingCore`). Аудит — аналогичный cross-cutting concern.
2. **Core остаётся чистым** — не добавляем зависимость на аудит в бизнес-логику.
3. **Покрытие MCP** — decorator оборачивает `CoreService` интерфейс, через который идут оба транспорта.
4. **Duration бесплатно** — decorator естественно измеряет время вызова.
5. **Метаданные** — decorator имеет доступ к request и response, чего достаточно для `extractMetadata`.

[ASSUMPTION: существующих данных в request/response достаточно для формирования полной AuditEntry — промежуточные шаги аудитировать не нужно]

[ASSUMPTION: MCP handler вызывает тот же `CoreService` интерфейс, и decorator автоматически покроет MCP-вызовы]

[ASSUMPTION: один decorator для аудита — достаточно, не нужен per-method fine-grained контроль включения/выключения аудита]

## Границы скоупа

- **Must-have (v1):**
  - `AuditCore` decorator, реализующий `CoreService`
  - Удаление `AuditInterceptor` из gRPC цепочки
  - Вспомогательная функция для извлечения caller IP из context
  - Прокидывание `AuditCore` в цепочку decorator'ов в `cmd/main.go`
  - Тесты на `AuditCore`
...
- [ASSUMPTION: request/response данных достаточно для формирования полной AuditEntry]
- [ASSUMPTION: MCP handler использует тот же `CoreService` интерфейс]
- [ASSUMPTION: порядок decorator'ов не критичен — `TracingCore(AuditCore(Core))` или `AuditCore(TracingCore(Core))` оба корректны]
