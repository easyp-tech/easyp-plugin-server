...
- В соответствии с решением пользователя Go-юнит-тесты не создаются.
- Верификация работы мигратора осуществляется интеграционным путем на этапе поднятия инфраструктуры и сборки плагинов.
...
### Интеграционное тестирование с использованием test_registry

Для тестирования мигратора используется директория `test_registry`, содержащая легковесные шаблоны тестовых плагинов:
- `test_registry/grpc/go/v1.5.1/`
- `test_registry/grpc-ecosystem/gateway/...`
- `test_registry/protocolbuffers/...`

Интеграционный тест выполняется на этапе сборки и поднятия инфраструктуры:

1. **Сборка тестовых плагинов**:
   Собрать бинарники из `test_registry` во временную папку `plugins/`.
2. **Запуск сервиса**:
   Запустить локальный сервер EasyP Service (например, с использованием `task up-minimal` или `go run ./cmd/easyp-svc/ service start`).
3. **Первичный импорт**:
   Выполнить команду миграции:
   `go run ./cmd/easyp-svc/ plugins migrate plugins/`
   *Ожидается:*
   - Корректная отрисовка динамического Mini-TUI (спиннеры, прогресс-бар).
   - Успешная регистрация 3-х тестовых плагинов (счетчик зарегистрированных = 3).
   - Подтверждение соответствия свойствам `Property 1` и `Property 2`.
4. **Повторный импорт**:
   Запустить миграцию повторно:
   `go run ./cmd/easyp-svc/ plugins migrate plugins/`
   *Ожидается:*
   - Пропуск всех плагинов со статусом `AlreadyExists`.
   - Итоговый отчет показывает: зарегистрировано = 0, пропущено (skipped) = 3, ошибок = 0 (свойство `Property 3`).
5. **Тестирование фильтрации**:
   Запустить миграцию с фильтром:
   `go run ./cmd/easyp-svc/ plugins migrate plugins/ --filter "grpc/*"`
   *Ожидается:*
   - Будут обработаны только плагины из группы `grpc`, остальные будут исключены (свойство `Property 4`).
6. **Тестирование неинтерактивного режима**:
   Запустить команду с перенаправлением вывода в файл или с флагом `--non-interactive`:
   `go run ./cmd/easyp-svc/ plugins migrate plugins/ --non-interactive`
   *Ожидается:*
   - В логах отсутствуют управляющие ANSI-последовательности и возвраты каретки `\r` (свойство `Property 5`).
